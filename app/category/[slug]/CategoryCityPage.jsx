...
export default function CategoryCityPage({ categoryName, cityName, content }) {
    ...
    return (<>
        ...
        >
            ...
            <h1 className="text-center font-serif leading-snug relative font-bold text-white text-2xl py-8 px-5 md:py-14 capitalize md:text-5xl">
        ...
        <section className="mx-auto lg:px-16 px-5 py-8 bg-gradient-to-b from-gray-50 to-gray-100">
            <h2 className="text-center text-xl md:text-4xl font-extrabold mb-4 leading-10">
                ...
                {content?.whyChoose(cityName).map((item, i) => {
                    return (
                        ...
                        >
                            ...
                            <p className="text-black text-sm relative z-10 leading-">
                                {item.desc}
                            </p>

                            <span className="absolute bottom-0 left-0 h-1 w-0 bg-blue-500 group-hover:w-full transition-all duration-500"></span>
                        </div>
                    );
                })}
            </div>
                ...
                {content?.Details(cityName)?.sections.map((section, i) => (
                    ...
                    >
                        ...
                        {section.points.map((point, j) => (
                            <p key={j} className="flex items-start gap-3 text-black text-sm">
                                <span>{point}</span>
                            </p>
                        ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                <div className="w-full">
                    <img
                        ...
                        style={content?.KeyFeatures?.imgPostion}
                    />
                </div>
        ...
        <section className="mx-auto lg:px-15 px-5 py-8 bg-white">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
                {content?.industries.map((i, idx) => {
                    const Icon = i.icon;
                    return (
                        <div key={idx} className="bg-white p-4 rounded-2xl shadow-md hover:shadow-2xl transition-all duration-300 text-center group">
                            ...
                            <h3 className="font-semibold text-lg mb-2 text-black">{i?.title}</h3>
        ...
        <section className="w-full bg-gray-900 text-white py-5">
            <div className="max-w-7xl mx-auto px-5 flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div>
                    <a href="tel:+919999402424" className="bg-white text-black px-6 py-3 rounded-lg font-semibold hover:bg-gray-100 transition">
                        Connect Now
                    </a>
                </div>
            </div>
        ...
        <section className="mx-auto lg:px-15 px-5 py-8 bg-gray-100" >
            ...
            <div className="max-w-4xl mx-auto space-y-4">
                {content?.faqs(cityName).items.map((i, idx) => (
                    <details className="group bg-gray-50 rounded-xl p-5 cursor-pointer" key={idx}>
        ...
        <section className="mx-auto lg:px-15 px-5 py-8" >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                <div className="w-full h-[26rem] flex items-center justify-center overflow-hidden">
                    <img
